...
use reth_revm::precompile::{
    PrecompileError, PrecompileOutput, PrecompileResult, PrecompileWithAddress, u64_to_address,
};

pub const L1SLOAD: PrecompileWithAddress =
    PrecompileWithAddress(u64_to_address(0x10001), l1sload_run);
...
mod tests {
    ...
    fn test_l1sload_succeeds_with_cached_storage() {
        ...
        let result = l1sload_run(&Bytes::from(input), SUFFICIENT_GAS);
        assert!(result.is_ok(), "Should succeed when storage value is cached");
