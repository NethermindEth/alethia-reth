...
//! [`clear_l1_storage`] (or the sibling
//! [`crate::precompiles::l1staticcall::clear_l1_staticcall_cache`]) at the top of every new block /
//! batch iteration. Tests use `#[serial]` to serialize global-state mutations.
...
mod tests {
    ...
    fn test_l1sload_succeeds_with_older_block() {
        ...
        assert!(
            result.is_ok(),
            "Should succeed for block within lookback range from l1_max_anchor"
        );
        assert_eq!(result.unwrap().bytes.as_ref(), &expected_value.0);
    }
    ...
    fn test_l1sload_rejects_block_near_anchor_but_far_from_l1_max_anchor() {
        ...
        assert!(
            result.is_err(),
            "Block at anchor should be rejected when too far from l1_max_anchor"
        );
    }
    ...
    fn test_anchor_block_id_context() {
        ...
        assert!(
            get_l1_max_anchor_block_id().is_none(),
            "L1 max-anchor context should be empty initially"
        );
        ...
        assert_eq!(
            ...
            Some(50),
            "Should retrieve the set l1 max-anchor block ID"
        );
