...
//! with one `(anchor, l1_max_anchor)` context at a time — the normal preflight and ZK-guest
//! pattern. Running two different `(anchor, l1_max_anchor)` contexts concurrently will silently
//! cross-contaminate cache hits.
//!
...
pub fn l1staticcall_run(input: &[u8], gas_limit: u64) -> PrecompileResult {
    ...
    // Block-range validation uses only `l1_max_anchor_block_id` for the `[l1_max_anchor − 256,
    // l1_max_anchor]` window. `anchor_block_id` participates only in the `l1_max_anchor <
    // anchor` invariant check and is *not* a bound of the lookback window.
    ...
    if is_reverted {
        // NMC parity: `GethLikeTxTracer.MarkAsFailed` returns `gas=0` for reverted
        // `debug_traceCall` invocations. Entries cached with `is_reverted=true` therefore
        // carry `gas_used=0` and empty `return_data`, matching surge-raiko's guest-side
        // revert assertion (`ensure!(w.gas_used == 0 && w.return_data.is_empty())`).
        //
        // Known limitation: `PrecompileError` cannot carry post-call gas, so reverted L1
        // calls still diverge from NMC's "charge gas on failure" path — this needs a
        // post-call gas refund threading through revm to fully match NMC semantics.
        debug!(
